...
public class UpgradeMenuBlock : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (GlobalMoney.moneyCount < UpgradeMenu.upgradeCosts[0])
        {
        ...
        if (GlobalMoney.moneyCount < UpgradeMenu.upgradeCosts[8] || BuildMenu.gameFinishedCheck == true)
        {
            ...
            if (BuildMenu.gameFinishedCheck == true)
            {
                spriteState = new SpriteState();
                spriteState.disabledSprite = max_buy;
                upgradeMenuButton9.spriteState = spriteState;
            }
